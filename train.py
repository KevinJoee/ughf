...
from net_u import net as myNet
import losses
...
parser = argparse.ArgumentParser(description='res')
parser.add_argument('--train_dir', default='./Datasets/train', type=str, help='Directory of train images')
parser.add_argument('--val_dir', default='./Datasets/test/Rain100L', type=str, help='Directory of validation images')
...
parser.add_argument('--session', default='res', type=str, help='session')
...
######### Resume ###########
if RESUME:
    path_chk_rest = utils.get_last_path('./checkpoints', 'model_best.pth')
...
train_loader = DataLoader(dataset=train_dataset, batch_size=batch_size, shuffle=True, num_workers=8, drop_last=False, pin_memory=False)

val_dataset = get_validation_data(val_dir, {'patch_size':patch_size})
val_loader = DataLoader(dataset=val_dataset, batch_size=batch_size, shuffle=False, num_workers=8, drop_last=False, pin_memory=False)
...
for epoch in range(start_epoch, num_epochs + 1):
    ...
    for i, data in enumerate(tqdm(train_loader), 0):
        ...
        target_ = data[0].cuda()
        input_  = data[1].cuda()
        m = abs(input_ - target_)
        m = kornia.geometry.transform.build_pyramid(m, 3)
        ...
        loss_mse = mse(restored[5], m[0])+mse(restored[4], m[1])+mse(restored[3], m[2])
        loss = loss_char + 0.01 * loss_fft + 0.05 * loss_edge + 0.001 * loss_mse
    ...
    if epoch % val_epochs == 0:
        ...
        for ii, data_val in enumerate((val_loader), 0):
            ...
            for res,tar in zip(restored[0], target):
                ...
                # psnr_val_rgb.append(math.sqrt(mse(img_as_float(res), img_as_float(tar))))
